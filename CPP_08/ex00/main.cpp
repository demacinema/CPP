...
{
	typename T::iterator it = container.begin(); // this line declares an iterator 'it' and initializes it to the beginning of the container
...
// An iterator, is a "pointer" that allows you to traverse elements in a container (incrementing, dereferencing and comparing)
// (like arrays, vectors, lists, etc.) without exposing the underlying structure of the container.
// A container could be described as a collection of elements, such as a list of numbers or a set of strings.
// Containers provide a way to store and organize data, while iterators provide a way to access and manipulate that data.
// Also, iterators are a fundamental part of the C++ Standard Template Library (STL).
// Theirs most important functions, have this syntax:
// iterator_name.operator++();  (for incrementing), iterator_name.operator*();  (for dereferencing), iterator_name.operator==();  (for comparison)
...
// If 'it' is equal to 'end', it indicates that the iterator has reached the end of the container, and a message is displayed accordingly.
