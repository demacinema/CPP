...
int Span::shortestSpan() const // finds the shortest span between any two numbers
{
	...
	std::set<int> sub; // to store the differences between consecutive elements. Using set to keep differences sorted.
...
Span::~Span()
{
	_data.clear(); // clear the multiset
}
...
// data.clear(); // clear the multiset, though not strictly necessary because of RAII
