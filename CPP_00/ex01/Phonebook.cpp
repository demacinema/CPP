...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/24 01:13:02 by demrodri          #+#    #+#             */
/*   Updated: 2024/10/10 20:48:22 by demacinema       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Phonebook::Phonebook(void)
{
	this->_index = 0;
	std::cout << \
		"Create a PHONEBOOK! Maximum 8 contacts!!" \
		<< std::endl;
	std::cout << \
		"Use the commands: ADD / SEARCH / EXIT" \
		<< std::endl;
}

Phonebook::~Phonebook(void)
{
	std::cout << "HASTA LA VISTA MUCHACHITO!!!" << std::endl;
}
...
void	Phonebook::add(void)
{
	...
	str = "";
	if (this->_index > 7)
		std::cout << "Warning: overwriting the contact of " << this->_contacts[this->_index % 8].get_fname() << std::endl;
	...
	while (!std::cin.eof() && str == "")
	{
		...
		if (std::getline(std::cin, str) && str != "")
		{
			...
			std::cout << this->_contacts[this->_index % 8].get_fname() << \
				" ADDED TO PHONEBOOK! [" << this->_index % 8 + 1 << "/8]" << std::endl;
		}
	}
	this->_index++;
}
